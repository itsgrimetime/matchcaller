    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                log(f"🔍 Found {len(tournaments)} tournaments in 18-hour window (12h ago to 6h from now)")
                ...
                log(f"🔍 Debug: Filtering for tournaments between {past_cutoff} and {future_cutoff}")
                log(f"🔍 Debug: This covers tournaments from 8 hours ago to 4 hours from now")
                
                for i, tournament in enumerate(tournaments):
                    if i < 5:  # Debug first 5 tournaments
                        log(f"🔍 Debug Tournament {i+1}: {tournament.get('name', 'Unknown')}")
                        log(f"   Slug: {tournament.get('slug')}")
                        log(f"   URL: https://www.start.gg/{tournament.get('slug', '')}")
                        log(f"   Start time: {tournament.get('startAt')} (vs cutoff: {past_cutoff})")
                    ...
                    if not start_time:
                        if i < 5:
                            log(f"   ❌ Skipped: No start time")
                        continue
                    if start_time < past_cutoff or start_time > future_cutoff:
                        if i < 5:
                            if start_time < now:
                                hours_ago = (now - start_time) / 3600
                                log(f"   ❌ Skipped: Started {hours_ago:.1f} hours ago (too long)")
                            else:
                                hours_future = (start_time - now) / 3600
                                log(f"   ❌ Skipped: Starts {hours_future:.1f} hours from now (too far)")
                        continue
                    ...
                    for j, event in enumerate(events):
                        ...
                        if i < 5:
                            log(f"     Event {j+1} '{event.get('name', 'Unknown')}': {len(sets)} sets, state: {event.get('state')}")
                            log(f"       Event slug: {event.get('slug')}")
                            log(f"       Event URL: https://www.start.gg/{event.get('slug', '')}")
                        
                        if not sets:
                            if i < 5:
                                log(f"       ❌ No active matches")
                            continue
                        ...
                        for match_set in sets:
                            if match_set.get("state") in [1, 2, 6]:  # Active match states
                                active_matches += 1
                        ...
                        if i < 5:
                            log(f"       Active matches: {active_matches}, Pools: {list(pools)}")
                        
                        # Check if the event/bracket has actually started
                        event_state = event.get("state")
                        if i < 5:
                            log(f"       Event state: {event_state} (1=Created, 2=Active, 3=Completed)")
                        ...
                        is_suitable = (
                            (event_state == 2 and active_matches > 0) or  # Properly started events
                            (active_matches >= 5)  # Or events with many active matches regardless of state
                        ...
                        if is_suitable:
                            ...
                            if i < 5:
                                if event_state == 2:
                                    log(f"       ✅ SUITABLE! Event ACTIVE (state {event_state}), {len(pools)} pools, {active_matches} matches")
                                else:
                                    log(f"       ✅ SUITABLE! Many active matches ({active_matches}), {len(pools)} pools, state {event_state}")
                        elif i < 5:
                            if active_matches == 0:
                                log(f"       ❌ No active matches found")
                            elif event_state != 2 and active_matches < 5:
                                log(f"       ❌ Event not active: state {event_state}, only {active_matches} matches (need 5+ or state 2)")
                            else:
                                log(f"       ❌ Not enough pools: {len(pools)} pools (need multiple pools or 3+ matches)")
                
                log(f"🎯 Found {len(suitable_tournaments)} suitable tournaments")
                
                if not suitable_tournaments:
                    log("❌ No suitable tournaments found")
                    log("💡 Looking for: tournaments with ACTIVE brackets (state 2) that have multiple pools and ongoing matches")
                    log("💡 Note: We skip tournaments with 'Created' brackets (state 1) that haven't started yet")
                    return None
                ...
                log(f"📊 {selected['pool_count']} pools, {selected['active_matches']} active matches")
                log(f"⏱️  Started {selected['hours_since_start']:.1f} hours ago")
                log(f"🎯 Event state: {selected['event_state']} (Active bracket with ongoing matches)")
...
def main():
    ...
    parser.add_argument("--find-active", action="store_true", help="Automatically find a random active tournament with multiple pools")
