...
class TournamentCloner:
    ...
    async def clone_tournament(self, event_slug: str) -> Optional[str]:
        ...
        log("✅ Tournament cloned successfully!")
            ...
            async with aiohttp.ClientSession() as session:
                while True:
                    ...
                    async with session.post(
                        ...
                        log(
                            f"📊 Page {page}: {len(sets_data)} sets (total so far: {len(all_sets)})"
                        )
    ...
    def _process_for_simulation(self, tournament_data: Dict, event_slug: str) -> Dict:
        ...
        for set_data in event_data["sets"]["nodes"]:
            ...
            match = {
                "id": set_data["id"],
                "display_name": set_data.get(
                    "fullRoundText", f"Round {set_data.get('round', '?')}"
                ),
                "player1": {"tag": player1},
                "player2": {"tag": player2},
                ...
                "phase_group": set_data.get("phaseGroup", {}).get(
                    "displayIdentifier", "Unknown"
                ),
                "phase_name": set_data.get("phaseGroup", {})
                .get("phase", {})
                .get("name", "Unknown Phase"),
                "round": set_data.get("round", 1),
                "entrant1_source": set_data.get("entrant1Source"),
                "entrant2_source": set_data.get("entrant2Source"),
            }
