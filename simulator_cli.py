...
# Add the matchcaller module to the path
...
def list_tournaments(args):
    ...
    if not tournaments:
        ...
        print("💡 Clone a tournament first: python simulator_cli.py clone --token TOKEN --slug SLUG")
        return 0
    ...
    if args.gui:
        ...
        def console_callback(state):
            ...
            for match in state["sets"][:5]:  # Show first 5 matches
                ...
                print(f"   {match['player1']['tag']} vs {match['player2']['tag']} - {status}")
            if len(state["sets"]) > 5:
...
def analyze_tournament(args):
    ...
    print(f"Duration: {data['duration_minutes']} minutes ({data['duration_minutes']/60:.1f} hours)")
    print("")
    ...
    print("Match States:")
    state_names = {1: "Created/Waiting", 2: "Ready/In Progress", 3: "Completed", 6: "In Progress", 7: "Invalid"}
...
Examples:
  ...
  python simulator_cli.py analyze tournament_data.json
    ...
    # Clone command
    clone_parser = subparsers.add_parser("clone", help="Clone a tournament from start.gg")
    clone_parser.add_argument("--token", required=True, help="start.gg API token")
    clone_parser.add_argument("--slug", required=True, help="Event slug (e.g., tournament/name/event/singles)")
    
    # List command
    list_parser = subparsers.add_parser("list", help="List cloned tournaments")
    
    # Simulate command
    simulate_parser = subparsers.add_parser("simulate", help="Run tournament simulation")
    simulate_parser.add_argument("file", help="Cloned tournament JSON file")
    simulate_parser.add_argument("--speed", type=float, default=60.0, help="Simulation speed multiplier (default: 60x)")
    simulate_parser.add_argument("--gui", action="store_true", help="Run with TUI interface")
