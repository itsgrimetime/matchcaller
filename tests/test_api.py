...
from matchcaller.matchcaller import TournamentAPI, MOCK_TOURNAMENT_DATA
...
class TestTournamentAPI:
    ...
    def test_init_with_slug(self):
        ...
        api = TournamentAPI(api_token="test_token", event_slug="tournament/test/event/singles")
    ...
    async def test_get_event_id_from_slug_not_found(self):
        ...
        with aioresponses() as m:
            ...
            result = await api.get_event_id_from_slug("tournament/nonexistent/event/singles")
    ...
    async def test_fetch_sets_with_slug_resolves_event_id(self):
        ...
        with aioresponses() as m:
            ...
            m.post(
                "https://api.start.gg/gql/alpha",
                payload=event_id_response,
                status=200
            )
            # Second call for sets data
            m.post(
                "https://api.start.gg/gql/alpha",
                payload=sets_response,
                status=200
            )
