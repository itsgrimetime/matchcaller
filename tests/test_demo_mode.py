...
from matchcaller.matchcaller import main, TournamentDisplay, MOCK_TOURNAMENT_DATA
...
class TestDemoMode:
    ...
    def test_demo_mode_forces_mock_data_parameters(self):
        ...
        test_args = ["--demo", "--token", "should_be_ignored", "--event", "should_be_ignored"]
        
        with patch('sys.argv', ['matchcaller.py'] + test_args), \
             patch('matchcaller.matchcaller.TournamentDisplay') as mock_app_class, \
             patch('time.sleep'):  # Skip sleep delays in tests
...
class TestLogging:
    ...
    @patch('matchcaller.matchcaller.log')
